...
<head>
  ...
  <link rel="stylesheet" href="proj_isshed.css">
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="text-center mb-12">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8 max-w-2xl mx-auto">
                    <div class="bg-white rounded-lg p-4 shadow-sm">
                        <div class="text-2xl font-bold text-green-600"><?php echo count(array_filter($timelineData, fn($item) => $item['category'] === 'completed')); ?></div>
                        <div class="text-sm text-gray-600">Completed</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 shadow-sm">
                        <div class="text-2xl font-bold text-yellow-500"><?php echo count(array_filter($timelineData, fn($item) => $item['category'] === 'in-progress')); ?></div>
                        <div class="text-sm text-gray-600">In Progress</div>
                    </div>
                    <div class="bg-white rounded-lg p-4 shadow-sm">
                        <div class="text-2xl font-bold text-blue-500"><?php echo count(array_filter($timelineData, fn($item) => $item['category'] === 'planned')); ?></div>
            ...
            <?php if (empty($timelineData)): ?>
                ...
                <div class="text-center py-16">
                    ...
                    <p class="text-gray-500 mb-6">Timeline events will appear here once they are added through the management interface.</p>
                    <a href="manage_timeline.php" class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors duration-200">
            ...
            <?php else: ?>
                ...
                <div class="relative">
                    ...
                    <div class="space-y-16">
                        ...
                        <div class="relative timeline-item" data-index="<?php echo $index; ?>">
                            <!-- Timeline Dot -->
                            <div class="timeline-dot <?php echo $item['category']; ?>"></div>

                            <!-- Content -->
                            <div class="timeline-item-container flex items-center <?php 
                                // Use database position or alternate based on index
                                $position = isset($item['position']) ? $item['position'] : ($index % 2 === 0 ? 'right' : 'left');
                                echo $position === 'right' ? 'flex-row-reverse' : 'flex-row'; 
                            ?>">
                                <!-- Content Card -->
                                <div class="timeline-content w-5/12 <?php echo $position === 'right' ? 'text-right pr-8' : 'text-left pl-8'; ?>">
                                    <div class="bg-white rounded-xl shadow-lg hover:shadow-xl p-8 border-l-4 <?php echo getStatusColorClass($item['category']); ?> transition-all duration-300 hover:-translate-y-1">
                                        <!-- Date Badge -->
                                        <div class="flex items-center gap-3 mb-4 <?php echo $position === 'right' ? 'justify-end' : 'justify-start'; ?>">
                                        ...
                                        <div class="mb-6">
                                            <img 
                                                ...
                                                class="w-full h-56 object-cover rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300"
                                                loading="lazy"
                                            />
                                        </div>
                                        <?php endif; ?>
                                        ...
                                        <!-- Status Badge -->
                                        <div class="flex <?php echo $position === 'right' ? 'justify-end' : 'justify-start'; ?>">
                                            <span class="<?php echo getStatusBadgeColor($item['category']); ?> text-sm px-4 py-2 rounded-full font-medium">
                                                <?php 
                                                $statusText = match($item['category']) {
                                                    'completed' => '✅ Completed',
                                                    'in-progress' => '🔄 In Progress',
                                                    'planned' => '📋 Planned',
                                                    default => ucfirst(str_replace('-', ' ', $item['category']))
                                                };
                                                echo $statusText;
                                                ?>
                                            </span>
                ...
                <div class="text-center mt-16">
                    <button 
                        ...
                        id="toggleButton"
                        class="inline-flex items-center gap-3 bg-gradient-to-r from-green-600 to-blue-600 hover:from-green-700 hover:to-blue-700 text-white px-8 py-4 rounded-xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-1"
                    >
                        <?php if ($showAll): ?>
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
